...
NSString *     const CLSpreadAnimationKeyScale = @"transform.scale";
NSTimeInterval const CLViscousityDuration = 0.15; // 粘滞动画时长控制
CGFloat        const CLBorderSpace = 10;
CGFloat        const CLSpreadDis = 70; //默认弹出距离
CGFloat        const CLAutoFitRadiousSpace = 0; // 自动适应items之间的弧度空隙
CGFloat        const CLRadiusStep = 5; // 递归变量递增值
BOOL            _iSHidden;  //是否是隐藏
UITapGestureRecognizer *_tapToMask;
#define kCLSCREEN_BOUNDS [[UIScreen mainScreen] bounds]

@interface CLSpredComponentry ()
...
@implementation CLSpredComponentry
...
- (void)panToSpread:(UIPanGestureRecognizer *)pan {
    ...
    if (_iSHidden==YES) { //如果是隐藏就弹出
        [UIView animateWithDuration:0.2 animations:^{
            if (self.frame.origin.x==-25) {
                self.frame = CGRectOffset(self.frame, 25, 0);
            }else{
                self.frame = CGRectOffset(self.frame, -25, 0);
            }
        ...
        return;
    }else if (_isSpreading == YES) { // 如果是展开就进来
...
- (void)spreadButtonUnborderFuncationCalFrame {
    CGFloat offset_x = self.frame.origin.x + self.frame.size.width - kCLSCREEN_BOUNDS.size.width;
    CGFloat offset_y = self.frame.origin.y + self.frame.size.height - kCLSCREEN_BOUNDS.size.height;
...
- (void)spreadButtonViscousityFuncationCalFrame {
    ...
    CGFloat cx = rect.origin.x - kCLSCREEN_BOUNDS.size.width / 2;
    if (cx > 0) {
        [UIView animateWithDuration:CLViscousityDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            self.frame = CGRectOffset(self.frame, kCLSCREEN_BOUNDS.size.width - rect.origin.x - rect.size.width, 0);
        } completion:NULL];
    } else {
        [UIView animateWithDuration:CLViscousityDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
...
- (void)addMaskLayer {
    ...
    UIView *maskView = [[UIView alloc] initWithFrame:kCLSCREEN_BOUNDS];
    ...
    _tapToMask = tapToMask;
    [maskView addGestureRecognizer:tapToMask];
}

- (void)tapToTempView {
    if (self.spreadButton.selected==YES) {
        [self spreadButtonDidClick:self.spreadButton];
    }
}
...
- (void)spreadScaleToSmallWithAnimated:(BOOL)animated {
    ...
    [self basicAnimationForSpreadButtonWithKeyPath:CLSpreadAnimationKeyScale fromValue:@1 toValue:@0.8];
}
...
- (void)spreadScaleToBigWithAnimated:(BOOL)animated {
    ...
    [self basicAnimationForSpreadButtonWithKeyPath:CLSpreadAnimationKeyScale fromValue:@0.8 toValue:@1];
}
...
- (void)spreadWithHandle:(void (^)(void))handle {
    ...
    if (self.frame.origin.x > 25) {//给展开点击图标数组排序
        self.subItems =  [[self.subItems reverseObjectEnumerator] allObjects];
    }
    //展开销毁定时器
    ...
    if (_iSHidden==YES) {
        ...
        [self openTimer];
        return;
    }else if (self.frame.origin.x==-25||self.frame.origin.x==([[UIScreen mainScreen] bounds].size.width-25)){
        [UIView animateWithDuration:0.2 animations:^{
            if (self.frame.origin.x==-25) {
                self.frame = CGRectOffset(self.frame, 25, 0);
            }else{
...
-(void)openTimer{
    //开启定时器
    if (!_isSpreading) {
        _timer = [NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(topMovingSuspensionBall:) userInfo:nil repeats:NO];
    }
}

#pragma mark - 悬浮球停止移动5秒收缩一半
-(void)topMovingSuspensionBall:(NSTimer *)sender{
    [self spreadButtonUnborderFuncationCalFrame];
...
- (BOOL)calInitialAngleWithTotalAngle:(CGFloat *)angle offsetAngle:(CGFloat *)sAngle {
    ...
    CGFloat lmax = l + _radius + CLBorderSpace;
    ...
    if (cp.y < lmax) {
        ...
        if (kCLSCREEN_BOUNDS.size.width - lmax < cp.x) {
            a2 = acos((kCLSCREEN_BOUNDS.size.width - cp.x - lmax + l) / l);
            at = M_PI_2 + a2;
            ac = M_PI_2 + a1 + a2;
        }
        if (cp.x < lmax) {
    ...
    if (kCLSCREEN_BOUNDS.size.height - lmax < cp.y) {
        a1 = acos((kCLSCREEN_BOUNDS.size.height - cp.y - lmax + l) / l);
        at = M_PI + a1;
        ac = a1 * 2;
...
- (CGFloat)autoCalSpreadDisWithStartAngle:(CGFloat *)sAngle totalAngle:(CGFloat *)tAngle {
    ...
    if (_autoAdjustToFitSubItemsPosition) {
        CGFloat rl = 2 * M_SQRT2 * _radius + CLAutoFitRadiousSpace;
        if (aAngle * _fixLength < rl) {
            _fixLength += CLRadiusStep;
            *sAngle = _offsetAngle;
            *tAngle = M_PI * 2;
